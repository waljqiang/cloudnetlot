...
use Modules\Home\Http\Requests\AccessTokenRequest;
...
class AuthController extends Controller{
    ...
    public function getToken(AccessTokenRequest $request){
        ...
        $credentials["status"] = config("public.user.status.enabled");
        $otherValidate = [
            "level" => config("public.user.level.child_guest")
        ];
        $token = auth("cloudnetlot")->attempt($credentials,true,$otherValidate);
