<?php
namespace Nova\Captcha;

class Captcha{
	protected $config =	array(
        'seKey'     =>  'waljqiang',   // 验证码加密密钥
    ...
    public function entry($id = '') {
        ...
        $res['code'] = $this->authcode(strtoupper(implode('', $code)));
     ...
     */
    public function check($code,$imageCode,$id = ''){
        $key = $this->authcode($this->seKey).$id;
       
        if(empty($code) || empty($imageCode)) {
            return false;
        }
        // session 过期
        if(time() - $code['create_time'] > $this->expire) {
            return false;
        }

        $imageCode = $this->authcode(strtoupper($imageCode.$id));
        if($imageCode == $code['code']) {
            $this->reset;
            return true;
        }
